...
describe("splitRecurrenceIcs", () => {
  ...
  it("keeps an UNTIL rule and the master's TZID on the tail", () => {
    ...
    const { after } = splitRecurrenceIcs(zoned, CUT, false, "split-zoned-tail")!;
    expect(after).toContain("UNTIL=20260601T000000Z");
    expect(after).toContain("DTSTART;TZID=America/Chicago:20260330T100000");
    expect(after).toContain("DTEND;TZID=America/Chicago:20260330T103000");
  });
