		...
		inputSchema: {
			...
			properties: {
				folder: {
					type: "string",
					description:
						"IMAP folder containing the email. Defaults to INBOX.",
				},
				uid: {
					type: "number",
		...
		inputSchema: {
			...
			properties: {
				...
				flags: {
					...
					items: { type: "string" },
					description:
						'Flags to set/unset (e.g., "\\Seen", "\\Flagged").',
				},
				action: {
					type: "string",
					enum: ["add", "remove"],
					description:
						'Whether to add or remove the flags. Defaults to "add".',
				},
			},
			required: ["uids", "flags"],
		},
		...
		inputSchema: {
			...
			properties: {
				...
				permanent: {
					type: "boolean",
					description:
						"If true, permanently delete instead of moving to Trash. Defaults to false.",
				},
			},
			required: ["uids"],
		},
		...
		inputSchema: {
			...
			properties: {
				...
				partId: {
					type: "string",
					description:
						"MIME part ID of the attachment (from get_email attachment metadata).",
				},
			},
			required: ["uid", "partId"],
		},
		...
		switch (name) {
			case "list_emails": {
				const query = args.query
					? parseSearchQuery(args.query as string)
					: {};
			...
			case "mark_email": {
				...
				return ok(
					JSON.stringify({ status: "updated", uids, flags, action }),
				);
			}
			...
			case "download_attachment": {
				...
				const attachment = await imapService.downloadAttachment(
					folder,
					uid,
					partId,
				);
